...
function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		elseif($type=='Type1' || $type=='TrueType')
		{
			...
			$this->_out('/fontsDescriptor '.($this->n+2).' 0 R');
			if(isset($font['diffn']))
			...
			// Descriptor
			$this->_newobj();
			$s = '<</Type /fontsDescriptor /fontsName /'.$name;
			foreach($font['desc'] as $k=>$v)
				$s .= ' /'.$k.' '.$v;
			if(!empty($font['file']))
				$s .= ' /fontsFile'.($type=='Type1' ? '' : '2').' '.$this->FontFiles[$font['file']]['n'].' 0 R';
			$this->_out($s.'>>');
			$this->_out('endobj');
		}
		else
